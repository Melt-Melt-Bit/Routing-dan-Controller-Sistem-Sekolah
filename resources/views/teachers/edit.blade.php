...
@section('content')
    ...
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        <a href="{{ route('teachers.index') }}"
            class="text-xs uppercase tracking-[0.15em] text-slate-400 hover:text-[#A16207]">&larr; Daftar Guru</a>
        <h1 class="font-display mt-2 text-3xl font-semibold text-[#16213A]">Ubah Data Guru</h1>
        <p class="mt-1 text-sm text-slate-500">Memperbarui data guru <span
    ...
    <form action="{{ route('teachers.update', ['teacher' => $teacher['id']]) }}" method="POST"
        ...
        <div>
            ...
            <select id="status" name="status"
                ...
                <option value="Tidak Aktif" {{ $teacher['status'] === 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif
                </option>
            </select>
        </div>
